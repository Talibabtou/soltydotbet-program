...
impl BettingState {
	pub fn place_bet(&mut self, bet: Bet) -> ProgramResult {
		...
		contract_state.bets.push(bet);
		Ok(())
	}

	fn validate_bet(&self, bet: &Bet) -> ProgramResult {
		if contract_state.current_phase != ContractPhase::Betting {
	...
	fn calculate_fees(&self, bet: &Bet) -> (u64, u64) {
		...
		if let Some(referral) = bet.referral {
			...
			let total_fee = financial_state.referral_fees.entry(referral).or_insert(0);
			*total_fee += referral_fee;
		} else {
			fee = (bet.amount * 4) / 100; // 4% house fee
		}
		financial_state.house_fee += fee;
